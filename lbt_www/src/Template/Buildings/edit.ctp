...
<?= $this->Html->link( $myBuildingCount === 1 ? __("Delete Building"): __("Delete Year"), "#", ['escape' => false, "class" => "delete pull-right mt-1 btn btn-danger", "data-id" => isset($building['id']) ? $building['id']:'', "data-title" => isset($building['id']) ? $building['name']:'']) ?>
<a href="<?= $this->Url->build(["controller" => "users", "controller" => "buildings"]); ?>"
   class="btn btn-default pull-right mt-1 mr-3"><?= __("Cancel"); ?></a>
<h2 class="mt-2"><?= __("Edit Building"); ?></h2>
...
$active_utility = isset($building) ? '' : '';
$building_details = isset($building) ? ' active' : ' active';
?>
...
var invalid_years = <?= json_encode($myBuildingYears) ;?>;
    ...
    $('.delete').on('click', function (e) {
        e.preventDefault();
        confirmDelete.find('.modal-title').html('<?=$myBuildingCount === 1 ? __('Delete Building'): __('Delete Building Year for');?> ' + $(this).attr("data-title") + ' <?=__(' ?');?>');
